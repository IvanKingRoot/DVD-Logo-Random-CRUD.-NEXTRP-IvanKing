...
crud.createWindow = function ()
	crud.ui.main = dgsCreateWindow(0, 0, screenW, screenH, "CRUD NEXTRP FOR IVANKING", false)
	dgsWindowSetCloseButtonEnabled(crud.ui.main, false)
	dgsWindowSetSizable(crud.ui.main, false)
	dgsWindowSetMovable(crud.ui.main, false)


	crud.ui.list = dgsCreateGridList(0, 0, screenW-200, screenH-25, false, crud.ui.main)
	...
	crud.ui.editName = dgsCreateEdit(screenW-190, 10, 180, 25, "Имя", false, crud.ui.main)
	crud.ui.editSurname = dgsCreateEdit(screenW-190, 40, 180, 25, "Фамилия", false, crud.ui.main)
	crud.ui.editAddres = dgsCreateEdit(screenW-190, 70, 180, 25, "Адрес", false, crud.ui.main)

	crud.ui.add = dgsCreateButton(screenW-190, 110, 180, 25, "Добавить", false, crud.ui.main)
	crud.ui.update = dgsCreateButton(screenW-190, 140, 180, 25, "Изменить", false, crud.ui.main)
	crud.ui.delete = dgsCreateButton(screenW-190, 170, 180, 25, "Удалить", false, crud.ui.main)

	crud.ui.editSearch = dgsCreateEdit(screenW-190, 220, 180, 25, "", false, crud.ui.main)
	crud.ui.buttonSearch = dgsCreateButton(screenW-190, 250, 180, 25, "Найти", false, crud.ui.main)

	crud.ui.refresh = dgsCreateButton(screenW-190, 300, 180, 25, "Обновить", false, crud.ui.main)
...
crud.onClick = function (btn, state)
	if btn == "left" and state == "down" then
		...
		if source == crud.ui.delete then
			...
			if selected ~= -1 then
				...
				crud.showAccepted("Вы уверены что хотите удалить строчку #"..ID.."?", function ()
					triggerServerEvent("deleteUser", localPlayer, ID)
				end)
			else
		...
		elseif source == crud.ui.add then
			...
			triggerServerEvent("onUserCreated", localPlayer, name, surname, address)
		elseif source == crud.ui.update then
			...
			if selected ~= -1 then 
				...
				triggerServerEvent("onUserUpdated", localPlayer, ID, name, surname, address)
			else
				crud.showMessage("Пожалуйста, выберите строчку!!")
			end
...
crud.showAccepted = function (text, funct1, funct2)
	...
	crud.ui.acceptedYes = dgsCreateButton(25, 110, 100, 25, "Да", false, crud.ui.acceptedWindow)
	crud.ui.acceptedNo = dgsCreateButton(175, 110, 100, 25, "Нет", false, crud.ui.acceptedWindow)
...
crud.startRefreshUsers = function ()
	triggerServerEvent("getUsers", localPlayer)
end
addEvent("startRefreshUsers", true)
addEventHandler("startRefreshUsers", localPlayer, crud.startRefreshUsers)

addEvent("refreshUsersDatabase", true)
addEventHandler("refreshUsersDatabase", localPlayer, function (data)
	crud.dataBase = data
	crud.refreshGrid()
end)
